module Decimacao #(
    ...
    // -----------------------------
    reg [7:0] guarda_pixel;
    reg [9:0] cord_x_pixel;
    reg [9:0] cord_y_pixel;
        ...
        if (reset) begin
            ...
            case(estado)
                ...
                PROCESSAMENTO: begin
                    ...
                    // Lógica de Pipeline: captura o pixel e suas coordenadas para o próximo ciclo
                    guarda_pixel <= pixel_in;
                    cord_x_pixel <= cont_x_orig;
                    cord_y_pixel <= cont_y_orig;

                    // Lógica de decimação: usa os dados do ciclo anterior
                    if ((cord_x_pixel % block_size_reg) == 0 && (cord_y_pixel % block_size_reg) == 0) begin
                        pixel_out <= guarda_pixel;
